...
public class Database extends SQLiteOpenHelper implements DatabaseInterface {
	...
	private static final String GAMES = "CREATE TABLE " + GAMES_TABLE_NAME
			+ " (" + GAMES_KEY_ID + " INTEGER PRIMARY KEY, "
			+ GAMES_KEY_VERSION + " REAL, " + GAMES_KEY_TITLE + " TEXT, "
			+ GAMES_KEY_DESCRIPTION + " TEXT, " + GAMES_KEY_GAME_ICON
			+ " TEXT, " + GAMES_KEY_OPERATOR_ICON + " TEXT, "
			+ GAMES_KEY_OPERATOR_NAME + " TEXT, " + GAMES_KEY_NUMBER_OF_PLAYERS
			+ " INTEGER, " + GAMES_KEY_NUMBER_OF_MAX_PLAYERS + " INTEGER, "
			+ GAMES_KEY_DIFFICULTY + " INTEGER, " + GAMES_KEY_CITY_NAME
			+ " TEXT, " + GAMES_KEY_START_DATE + " INTEGER, "
			+ GAMES_KEY_WINNING_STRATEGY + " TEXT, " + GAMES_KEY_END_DATE
			+ " INTEGER, " + GAMES_KEY_COMMENTS + " TEXT, " + GAMES_KEY_REWARD
			+ " TEXT, " + GAMES_KEY_PRIZE_INFO + " TEXT, "
			+ GAMES_KEY_DETAILS_LINK + " TEXT" + ")";

	private static final String USER = "CREATE TABLE " + USER_TABLE_NAME + " ("
			+ USER_KEY_EMAIL + " TEXT PRIMARY KEY, " + USER_KEY_PASSWORD
			+ " TEXT, " + USER_KEY_DISPLAY_NAME + " TEXT," + USER_KEY_AVATAR
			+ " TEXT" + ")";

	private static final String USER_GAMES_SPECIFIC = "CREATE TABLE "
			+ USER_GAMES_SPECIFIC_TABLE_NAME + " ("
			+ USER_GAMES_SPECIFIC_KEY_EMAIL + " TEXT, "
			+ USER_GAMES_SPECIFIC_KEY_GAME_ID + " INTEGER, "
			+ USER_GAMES_SPECIFIC_KEY_RANK + " INTEGER, "
			+ USER_GAMES_SPECIFIC_KEY_GAME_ACTIVE_OBSERVED + " INTEGER, "
			+ "PRIMARY KEY (" + USER_GAMES_SPECIFIC_KEY_EMAIL + ", "
			+ USER_GAMES_SPECIFIC_KEY_GAME_ID + ")" + " FOREIGN KEY ("
			+ USER_GAMES_SPECIFIC_KEY_EMAIL + ") " + "REFERENCES "
			+ USER_TABLE_NAME + " (" + USER_KEY_EMAIL + ") " + " FOREIGN KEY ("
			+ USER_GAMES_SPECIFIC_KEY_GAME_ID + ") " + "REFERENCES "
	...
	public boolean insertGameInfo(UrbanGame game) {
		...
		if (isDataOk) {
			...
			boolean isInDB = db.query(GAMES_TABLE_NAME,
					new String[] { GAMES_KEY_ID }, GAMES_KEY_ID + "=?",
					new String[] { game.getID().longValue() + "" }, null, null,
					null).moveToFirst();

			boolean isInsertOK;
			if (isInDB) {
				isInsertOK = db.update(GAMES_TABLE_NAME, values, GAMES_KEY_ID
						+ "=?", new String[] { game.getID().longValue() + "" }) != 0;
			} else {
	...
	private List<UrbanGameShortInfo> getAllGamesShortInfoParameterized(
			String orderLimitQuery) {
		List<UrbanGameShortInfo> gameList = null;

		String query = "SELECT " + GAMES_KEY_ID + ", " + GAMES_KEY_TITLE + ", "
				+ GAMES_KEY_GAME_ICON + ", " + GAMES_KEY_OPERATOR_ICON + ", "
				+ GAMES_KEY_OPERATOR_NAME + ", " + GAMES_KEY_NUMBER_OF_PLAYERS
				+ ", " + GAMES_KEY_NUMBER_OF_MAX_PLAYERS + ", "
				+ GAMES_KEY_CITY_NAME + ", " + GAMES_KEY_START_DATE + ", "
				+ GAMES_KEY_END_DATE + ", " + GAMES_KEY_REWARD + ", "
				+ GAMES_KEY_DETAILS_LINK + " FROM " + GAMES_TABLE_NAME
				+ orderLimitQuery;
	...
	public List<UrbanGameShortInfo> getAllGamesShortInfoOrderedByStartTime(
		...
		String orderingLimitingQuery = " ORDER BY " + GAMES_KEY_START_DATE
				+ " LIMIT " + beforeFirst + ", " + howMany;
	...
	public UrbanGameShortInfo getGameShortInfo(Long gameID) {
		SQLiteDatabase db = this.getReadableDatabase(DATABASE_PASS);
		String[] gamesColumns = { GAMES_KEY_ID, GAMES_KEY_TITLE,
				GAMES_KEY_GAME_ICON, GAMES_KEY_OPERATOR_ICON,
				GAMES_KEY_OPERATOR_NAME, GAMES_KEY_NUMBER_OF_PLAYERS,
				GAMES_KEY_NUMBER_OF_MAX_PLAYERS, GAMES_KEY_CITY_NAME,
				GAMES_KEY_START_DATE, GAMES_KEY_END_DATE, GAMES_KEY_REWARD,
				GAMES_KEY_DETAILS_LINK };

		Cursor cursor = db.query(GAMES_TABLE_NAME, gamesColumns, GAMES_KEY_ID
				+ "=?", new String[] { gameID.longValue() + "" }, null, null,
				null, null);
	...
	public UrbanGame getGameInfo(Long gameID) {
		...
		String[] gamesColumns = { GAMES_KEY_ID, GAMES_KEY_TITLE,
				...
				GAMES_KEY_NUMBER_OF_MAX_PLAYERS, GAMES_KEY_CITY_NAME,
				GAMES_KEY_START_DATE, GAMES_KEY_END_DATE, GAMES_KEY_REWARD,
				GAMES_KEY_DETAILS_LINK, GAMES_KEY_VERSION,
				GAMES_KEY_WINNING_STRATEGY, GAMES_KEY_DIFFICULTY,
				GAMES_KEY_PRIZE_INFO, GAMES_KEY_DESCRIPTION, GAMES_KEY_COMMENTS };
	...
	public boolean updateGameShortInfo(UrbanGameShortInfo game) {
		...
		if (isDataOk) {
			...
			updateOK = db.update(GAMES_TABLE_NAME, values, GAMES_KEY_ID + "=?",
					new String[] { game.getID().longValue() + "" }) == 1;
		}
		db.close();
	...
	public boolean updateGame(UrbanGame game) {
		...
		if (isDataOk) {
			...
			isUpdateOk = db.update(GAMES_TABLE_NAME, values, GAMES_KEY_ID
					+ "=?", new String[] { game.getID().longValue() + "" }) == 1;
		}
		db.close();
	...
	public boolean deleteGameInfoAndShortInfo(Long gameID) {
		SQLiteDatabase db = this.getWritableDatabase(DATABASE_PASS);
		boolean isSucessful = db.delete(GAMES_TABLE_NAME, GAMES_KEY_ID + "=?",
	...
	private boolean areShortGameInfoFieldsOK(UrbanGameShortInfo game) {
		boolean isOK = game.getTitle() != null
				...
				&& (game.getStartDate() != null && game.getStartDate().after(
						new GregorianCalendar(2012, 12, 31).getTime()))
				&& game.getEndDate() != null;
		return isOK;
	}

	private boolean areGameInfoFieldsOK(UrbanGame game) {
		boolean isOK = areShortGameInfoFieldsOK(game.getPrimaryInfo())
				&& game.getGameVersion() != null
				&& game.getGameVersion() >= 0.0
				&& (game.getDifficulty() == null || game.getDifficulty() >= 0)
				&& (game.getDifficulty() == null || game.getDifficulty() <= 5)
	...
	private UrbanGameShortInfo gameShortInfoFromCursor(Cursor cursor) {
		UrbanGameShortInfo game = new UrbanGameShortInfo(cursor.getLong(0),
				cursor.getString(1), cursor.getString(4), cursor.getInt(5),
				cursor.getInt(6), longToDate(cursor.getLong(8)),
				longToDate(cursor.getLong(9)),
				stringToBoolean(cursor.getString(10)), cursor.getString(7),
				cursor.getString(2), cursor.getString(3), cursor.getString(11));
		return game;
	}
	...
	private void putUrbanGameShortInfoInValues(UrbanGameShortInfo game,
	...
	public Player getPlayer(String email) {
		SQLiteDatabase db = this.getReadableDatabase(DATABASE_PASS);
		String[] playerColumns = { USER_KEY_EMAIL, USER_KEY_DISPLAY_NAME,
				USER_KEY_AVATAR, USER_KEY_PASSWORD };

		Cursor cursor = db.query(USER_TABLE_NAME, playerColumns, USER_KEY_EMAIL
				+ "=?", new String[] { email }, null, null, null, null);

		Player player;
		if (cursor != null && cursor.moveToFirst()) {
			player = new Player(cursor.getString(0), cursor.getString(3),
	...
	public boolean updatePlayer(Player player) {
		...
		if (isDataOk) {
			...
			updateOK = db.update(USER_TABLE_NAME, values,
					USER_KEY_EMAIL + "=?", new String[] { player.getEmail() }) == 1;
		}
		db.close();
		return isDataOk && updateOK;
	}
	...
	public boolean deletePlayer(String email) {
		SQLiteDatabase db = this.getWritableDatabase(DATABASE_PASS);
		boolean isSucessful = db.delete(USER_TABLE_NAME, USER_KEY_EMAIL + "=?",
				new String[] { email }) != 0;
		db.close();
		return isSucessful;
	}
	...
	public boolean insertUserGameSpecific(PlayerGameSpecific playerGameSpecific) {
		...
		isDataOk = playerGameSpecific.getPlayerEmail() != null
				&& playerGameSpecific.getGameID() != null;

		if (isDataOk) {
			ContentValues values = new ContentValues();
			values.put(USER_GAMES_SPECIFIC_KEY_EMAIL,
					playerGameSpecific.getPlayerEmail());
			values.put(USER_GAMES_SPECIFIC_KEY_GAME_ID,
					playerGameSpecific.getGameID());
			values.put(USER_GAMES_SPECIFIC_KEY_RANK,
					playerGameSpecific.getRank());
			values.put(USER_GAMES_SPECIFIC_KEY_GAME_ACTIVE_OBSERVED,
					playerGameSpecific.getState());

			boolean isInsertOK = db.insert(USER_GAMES_SPECIFIC_TABLE_NAME,
					null, values) != -1;
			db.close();
	...
	public PlayerGameSpecific getUserGameSpecific(String email, Long gameID) {
		SQLiteDatabase db = this.getReadableDatabase(DATABASE_PASS);
		String[] playerGamesSpecificColumns = { USER_GAMES_SPECIFIC_KEY_EMAIL,
				USER_GAMES_SPECIFIC_KEY_GAME_ID, USER_GAMES_SPECIFIC_KEY_RANK,
				USER_GAMES_SPECIFIC_KEY_GAME_ACTIVE_OBSERVED };

		Cursor cursor = db.query(USER_GAMES_SPECIFIC_TABLE_NAME,
				playerGamesSpecificColumns, USER_GAMES_SPECIFIC_KEY_EMAIL
						+ "=? AND " + USER_GAMES_SPECIFIC_KEY_GAME_ID + "=?",
				new String[] { email, gameID.longValue() + "" }, null, null,
				null, null);

		PlayerGameSpecific playerGameSpecific;
		if (cursor != null && cursor.moveToFirst()) {
			playerGameSpecific = new PlayerGameSpecific(cursor.getInt(2),
					cursor.getString(0), cursor.getLong(1), cursor.getInt(3));
		} else {
	...
	public boolean updateUserGameSpecific(PlayerGameSpecific playerGameSpecific) {
		...
		boolean isDataOk = playerGameSpecific.getPlayerEmail() != null
				&& playerGameSpecific.getGameID() != null;
		boolean updateOK = false;
		if (isDataOk) {
			ContentValues values = new ContentValues();
			...
			updateOK = db.update(USER_GAMES_SPECIFIC_TABLE_NAME, values,
					USER_GAMES_SPECIFIC_KEY_EMAIL + "=? AND "
							+ USER_GAMES_SPECIFIC_KEY_GAME_ID + "=?",
					new String[] { playerGameSpecific.getPlayerEmail(),
	...
	public boolean deleteUserGameSpecific(String email, Long gameID) {
		SQLiteDatabase db = this.getWritableDatabase(DATABASE_PASS);
		boolean isSucessful = db.delete(USER_GAMES_SPECIFIC_TABLE_NAME,
				USER_GAMES_SPECIFIC_KEY_EMAIL + "=? AND "
						+ USER_GAMES_SPECIFIC_KEY_GAME_ID + "=?", new String[] {
						email, gameID.longValue() + "" }) != 0;
		db.close();
		return isSucessful;
	}
	...
	public boolean wipeOutUserData(String email) {
		...
		db.beginTransaction();
		boolean isSucessful = db.delete(USER_GAMES_SPECIFIC_TABLE_NAME,
				USER_GAMES_SPECIFIC_KEY_EMAIL + "=?", new String[] { email }) != 0;
		boolean isSuccessful2 = db.delete(USER_TABLE_NAME, USER_KEY_EMAIL
				+ "=?", new String[] { email }) != 0;
