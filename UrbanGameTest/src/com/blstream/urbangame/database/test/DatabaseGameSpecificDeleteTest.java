...
public class DatabaseGameSpecificDeleteTest extends AndroidTestCase {
	...
	private void prepareDataUserSpecific(Integer rank, String email,
			Long gameID, Integer status) {
		PlayerGameSpecific player = new PlayerGameSpecific(rank, email, gameID,
				status);
	...
	private void prepareDataUser(String email, String pass, String displayName,
			String avatarBase64) {
	...
	public void testOneItemInDatabaseNoMatch() {
		prepareDataUserSpecific(2, "em@em.em", 1L,
	...
	public void testManyItemsInDatabaseNoMatch() {
		for (int i = 0; i < 30; i++) {
			prepareDataUserSpecific(i, "em@em.em" + i, (long) i,
					i % 2 == 0 ? PlayerGameSpecific.GAME_ACTIVE
	...
	public void testManyItemsInDatabaseMatch() {
		...
		boolean isSuccessful = database.deleteUserGameSpecific("em@em.em" + 13,
				13L);
		assertTrue(isSuccessful);
	}
	...
	public void testWipeOutUserData() {
		...
		for (int i = 0; i < 30; i++) {
			prepareDataUserSpecific(i * 7, "em@em.em", (long) i,
					i % 2 == 0 ? PlayerGameSpecific.GAME_ACTIVE
							: PlayerGameSpecific.GAME_OBSERVED);
		}
