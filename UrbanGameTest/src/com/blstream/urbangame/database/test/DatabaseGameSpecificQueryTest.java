...
public class DatabaseGameSpecificQueryTest extends AndroidTestCase {
	...
	private void prepareData(Integer rank, String email, Long gameID,
			Integer status) {
	...
	public void testManyItemsInDatabaseNoMatch() {
		for (int i = 0; i < 30; i++) {
			prepareData(i + 1, "em@em.em" + i, (long) i * 2,
	...
	public void testManyItemsInDatabaseMatch() {
		...
		int tested = 13;
		PlayerGameSpecific p = database.getUserGameSpecific("em@em.em" + tested,
				(long) tested * 2);
		assertNotNull(p);
		...
		assertEquals(tested % 2 == 0 ? PlayerGameSpecific.GAME_ACTIVE
				: PlayerGameSpecific.GAME_OBSERVED, p.getState().intValue());
	}
	/* ------------------------ USER QUERIES END ------------------------ */
}