...
public class DatabaseGamesQueryTest extends AndroidTestCase {
	...
	public void testOneItemInDatabase() {
		prepareDataGameShort(3L, "title", "operatorName", 2, 50, new Date(),
				new Date(), true, "location", "gamelogoBase64",
	...
	public void testManyItemsInDatabaseOrderedByStartTime() {
		...
		List<UrbanGameShortInfo> gamesList = database
				.getAllGamesShortInfoOrderedByStartTime(0, 30);
		assertNotNull(gamesList);
		assertEquals(gamesList.size(), 30);
		...
		for (int i = 1; i < gamesList.size(); i++) {
			areSorted = areSorted
					&& d.getTime() <= gamesList.get(i).getStartDate().getTime();
		}
	...
	public void testManyItemsInDatabaseOrderedByStartTimeAndPagination() {
		...
		List<UrbanGameShortInfo> gamesList = database
				.getAllGamesShortInfoOrderedByStartTime(10, 10);
		assertNotNull(gamesList);
	...
	public void testManyItemsInDatabaseOrderedByStartTimeAndPaginationEndData() {
		...
		for (int i = 0; i < 30; i++) {
			c.set((int) Math.random() * 2000,
					(int) Math.random() * 100 % 12 + 1,
					(int) Math.random() * 100 % 24 + 1,
					(int) Math.random() * 100 % 23 + 1,
					(int) Math.random() * 100 % 58 + 1);
			prepareDataGameShort((long) i, "title", "operatorName", 2, 50,
					new Date(), new Date(), true, "location", "gamelogoBase64",
					"operatorlogoBase64", "detailsLink");
		}
		List<UrbanGameShortInfo> gamesList = database
				.getAllGamesShortInfoOrderedByStartTime(24, 30);
		assertNotNull(gamesList);
	...
	public void testOneItemInDatabaseSingleNoMatchSHORT() {
		prepareDataGameShort(2L, "title", "operatorName", 2, 50, new Date(),
				new Date(), true, "location", "gamelogoBase64",
				"operatorlogoBase64", "detailsLink");
	...
	public void testManyItemsInDatabaseSingleNoMatch() {
		...
		prepareDataGame(2L, 2.4, "title", "Operator Name", "winningStrategy",
				34, 50, startDate, endDate, 2, true, "prizesInfo",
				"description", "gamelogoBase64", "operatorlogoBase64",
				"comments", "location", "detaisLink");
	...
	public void testOneItemInDatabaseSingleMatch() {
		...
		c.set(Calendar.MILLISECOND, 0);
		Date endDate = c.getTime();
		prepareDataGame(1L, 2.4, "title", "Operator Name", "winningStrategy",
				34, 50, startDate, endDate, 2, true, "prizesInfo",
				"description", "gamelogoBase64", "operatorlogoBase64",
				"comments", "location", "detailsLink");
	...
	public void testManyItemsInDatabaseSingleMatch() {
		...
		assertNotNull(game);
		assertEquals((long) testedGame, game.getID().longValue());
		...
		assertEquals("operatorlogoBase64" + testedGame,
				game.getOperatorLogoBase64());
	...
	public void testManyItemsInDatabaseSingleMatchSHORT() {
		...
		for (int i = 0; i < 30; i++) {
			...
			c.set(i + 2014, (i + 1) % 12 + 1, i % 10 + 4);
			Date end = c.getTime();
			prepareDataGameShort((long) i, "title" + i, "operatorName" + i,
					i * 3, i * 10, start, end, i % 2 == 0, "location" + i,
					"gamelogoBase64" + i, "operatorlogoBase64" + i,
					"detailsLink" + i);
