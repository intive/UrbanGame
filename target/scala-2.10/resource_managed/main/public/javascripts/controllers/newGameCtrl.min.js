...
$scope.geolocationBound=true;return console.log("geoagain")}});$("#locationInput").on("click",function(){return $("#locationInput").bind("propertychange blur input paste",function(event){return setTimeout(function(){return $scope.game.loc=$("#locationInput").val()},500)})});return $("#startTime, #startDate, #endTime, #endDate").bind("input blur",function(event){var from;var to;if($scope.game.startDate instanceof Date&&$scope.game.endDate instanceof Date&&$scope.form.gStartTime.$valid&&$scope.form.gEndTime.$valid){from=
new Date($scope.game.startDate.getTime());to=new Date($scope.game.endDate.getTime());from.setHours(parseInt($scope.game.startTime.split(":")[0]),parseInt($scope.game.startTime.split(":")[1]));to.setHours(parseInt($scope.game.endTime.split(":")[0]),parseInt($scope.game.endTime.split(":")[1]));if(from.getTime()>to.getTime())return $scope.form.$setValidity("dates",false);else return $scope.form.$setValidity("dates",true)}else return $scope.form.$setValidity("dates",true)})},200)};return $(function(){console.log("switched to tab1");
return $scope.onTab1Switch()})}]);time_regexp=/^(20|21|22|23|[01]\d|\d)(:[0-5]\d)$/;app.directive("time",function(){return{require:"ngModel",link:function(scope,elm,attr,ctrl){return ctrl.$parsers.unshift(function(viewValue){if(time_regexp.test(viewValue)){ctrl.$setValidity("time",true);return viewValue}else{ctrl.$setValidity("time",false);return"undefined"}})}}});