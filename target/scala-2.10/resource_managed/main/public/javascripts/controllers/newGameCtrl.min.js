...
$scope.resetDefaultTaskTemplate=function(){var additionalCond;$scope.task={type:"ABC",name:"",description:"",locations:[],maxattempts:"",maxpoints:"",penalty:"",repeat:false,answers:[{symbol:"A",text:"",points:null},{symbol:"B",text:"",points:null},{symbol:"C",text:"",points:null},{symbol:"D",text:"",points:null}]};additionalCond=false;return $scope.taskForm.$setValidity("locations",false)};$scope.resetAttempts=function(){return $scope.task.maxattempts=""};$scope.addAnswer=function(){var symbol;symbol=
String.fromCharCode(65+$scope.task.answers.length);return $scope.task.answers.push({symbol:symbol,text:"",points:null,correct:true})};$scope.removeAnswer=function(index){return $scope.task.answers.splice(index,1)};$scope.addLocation=function(){var lat;var lng;var radius;lat=Number($("#latitude").val());lng=Number($("#longitude").val());radius=Number($("#radius").val());if(!isNaN(lat)&&!isNaN(lng)&&!isNaN(radius)){$scope.task.locations.push({lat:lat,lng:lng,radius:radius});$scope.repaintLocations();
return $scope.taskForm.$setValidity("locations",true)}};$scope.removeLocations=function(){var i;var options;options=document.getElementById("locationsList").options;for(i=options.length-1;i!==-1;){if(options[i].selected)$scope.task.locations.splice(i,1);i--}$scope.repaintLocations();if($scope.task.locations.length===0)return $scope.taskForm.$setValidity("locations",false)};$scope.additionalCond=false;$(function(){fillGameModel();return $scope.resetDefaultTaskTemplate()});return $scope.$watch("[game.winningNum, game.playersNum]",
