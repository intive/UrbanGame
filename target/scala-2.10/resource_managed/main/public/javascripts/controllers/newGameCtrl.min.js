...
$scope.resetDefaultTaskTemplate=function(){var additionalCond;$scope.task={type:"ABC",name:"",description:"",locations:[],maxattempts:null,maxpoints:null,penalty:null,repeat:false,answers:[{symbol:"A",text:"",points:null},{symbol:"B",text:"",points:null},{symbol:"C",text:"",points:null},{symbol:"D",text:"",points:null}]};return additionalCond=false};$scope.addAnswer=function(){var symbol;symbol=String.fromCharCode(65+$scope.task.answers.length);return $scope.task.answers.push({symbol:symbol,text:"",
points:null,correct:true})};$scope.removeAnswer=function(index){return $scope.task.answers.splice(index,1)};$scope.addLocation=function(){var lat;var lng;var radius;lat=Number($("#latitude").val());lng=Number($("#longitude").val());radius=Number($("#radius").val());console.log(lat+" "+lng+" "+radius);if(!isNaN(lat)&&!isNaN(lng)&&!isNaN(radius)){console.log("wtf");$scope.task.locations.push({lat:lat,lng:lng,radius:radius});return $scope.repaintLocations()}};$scope.removeLocations=function(){var i;
var options;options=document.getElementById("locationsList").options;for(i=options.length-1;i!==-1;){if(options[i].selected)$scope.task.locations.splice(i,1);i--}return $scope.repaintLocations()};$scope.additionalCond=false;$(function(){fillGameModel();return $scope.resetDefaultTaskTemplate()});$scope.$watch("[game.winningNum, game.playersNum]",function(){if(_.isUndefined($scope.game.winningNum)||$scope.game.winningNum<1)return $scope.form.$setValidity("morewinnersthanplayers",false);else if($scope.game.playersNum!==
null)if($scope.game.playersNum>$scope.game.winningNum)return $scope.form.$setValidity("morewinnersthanplayers",true);else return $scope.form.$setValidity("morewinnersthanplayers",false);else return $scope.form.$setValidity("morewinnersthanplayers",true)},true);return console.log($scope)}]);